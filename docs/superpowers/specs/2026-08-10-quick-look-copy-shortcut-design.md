...
Because Quick Look's out-of-process host keeps the arrow cursor even when WebKit reports selectable text, the extension registers AppKit's native I-beam cursor rect on its web view. It deliberately leaves the generated Markdown HTML and `WKWebViewConfiguration` unchanged. WebKit's more specific interactive descendants can continue supplying their own cursor behavior. The cursor rect does not change selection painting or the main app.
